...
1. 加锁前（Lock Before）：
   ...
   - 在加锁前，通常需要记录当前线程即将获取的锁信息，包括锁的标识符、线程的标识符等。

   - 更新数据结构或记录以表示当前线程即将获取的锁，以便后续的死锁检测。

   - 可能需要检查该次加锁是否会导致死锁，例如检查是否存在环路等情况。

*/
//判断女生是否单身：是，正常进行；不是，查找其男朋友id
void lock_before(uint64_t tid, uint64_t lockaddr) {
	...
	for (idx = 0;idx < tg->lockidx;idx ++) {
		...
		if (tg->locklist[idx].lock_id == lockaddr) { // 
			...
			struct source_type from;   //线程
			from.id = tid;
			from.type = PROCESS;
			add_vertex(from);

			struct source_type to;    //锁
			to.id = tg->locklist[idx].id;
			to.type = PROCESS;
			add_vertex(to);

			
			tg->locklist[idx].degress ++;   //线程每持有一个锁，锁的度数+1
...
2. 加锁后（Lock After）：
   ...
   - 在成功获取锁之后，通常需要更新相关数据结构，表示当前线程已经成功获取了锁。

   - 可能需要检查是否存在其他线程等待当前线程持有的锁，以便进行相应的死锁预防措施。

   - 可能需要更新锁的状态或度数等信息，以反映当前锁的状态变化。
*/
//关系表中增加一条关系
void lock_after(uint64_t tid, uint64_t lockaddr) {
	int idx = 0;
	if (-1 == (idx = search_lock(lockaddr))) {   //锁列表中不存在，新建
...
3. 解锁后（Unlock After）：
   ...
   - 在成功释放锁之后，通常需要更新数据结构，表示当前线程已释放该锁。
*/
